...
class PlatformMediaCaptureService implements MediaCaptureService {
  ...
  @override
  Future<void> startVoiceRecording() => _channel.invokeMethod<void>('startVoiceRecording');

  @override
  Future<LocalMediaFile?> stopVoiceRecording() async {
    final path = await _channel.invokeMethod<String>('stopVoiceRecording');
    return path == null ? null : LocalMediaFile(path: path, mimeType: 'audio/mp4');
  }

  @override
  Future<void> cancelVoiceRecording() => _channel.invokeMethod<void>('cancelVoiceRecording');

  @override
  Future<LocalMediaFile> createDemoPhoto() async {
    final file = File('${Directory.systemTemp.path}${Platform.pathSeparator}kalasetu_demo_photo.jpg');
    if (!await file.exists()) await file.writeAsBytes(const [255, 216, 255, 217]);
  ...
  Future<LocalMediaFile?> _photo(String method) async {
    ...
    return path == null ? null : LocalMediaFile(path: path, mimeType: _photoMimeType(path));
  }
